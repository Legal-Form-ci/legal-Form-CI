...
export const buildSystemPrompt = (imageMode: ImageMode) => {
  ...
  return `Tu es un moteur éditorial premium pour Legal Form SARL (Côte d'Ivoire).\n\nOBJECTIF :\n- Transformer une saisie libre (même un mot) en article complet et publiable\n- Ton naturel, professionnel, humain, sans style robotique\n- Aucun HTML visible, uniquement Markdown\n\nRÈGLES :\n1) Titre impactant en MAJUSCULES\n2) Résumé court et engageant (max 200 caractères)\n3) Contenu structuré : intro claire, sections logiques, sous-titres (##, ###), paragraphes aérés, listes pertinentes, tableaux si utile\n4) Catégorie parmi : Fiscalité, Juridique, Entrepreneuriat, Actualités, Formation, Conseils, Événements, Annonces\n5) Tags pertinents séparés par des virgules\n6) Meta description SEO (max 160 caractères)\n7) Slug propre en minuscules avec tirets\n8) Minimum 700 mots\n9) Style journalistique rigoureux et factuel\n10) ${mediaInstruction}\n\nRetourne UNIQUEMENT du JSON valide avec la structure exacte :\n{\n  "title": "TITRE EN MAJUSCULES",\n  "excerpt": "Résumé court",\n  "category": "Catégorie",\n  "formattedContent": "Contenu complet en Markdown",\n  "tags": "tag1, tag2, tag3",\n  "meta_description": "Description SEO",\n  "slug": "slug-url",\n  "author_name": "Legal Form SARL"\n}`;
};

export const callAiGateway = async (
  apiKey: string,
  ...
  if (!response.ok) {
    if (response.status === 429) {
      throw new HttpError(429, "Limite de requêtes atteinte, réessayez dans quelques secondes");
    }

    if (response.status === 402) {
      throw new HttpError(402, "Crédits IA insuffisants");
    }
...
export const generateArticle = async (
  ...
  const aiResponse = await callAiGateway(apiKey, {
    ...
    messages: [
      ...
      {
        role: "user",
        content: `Voici le contenu source à enrichir :\n\n${content}`,
      },
    ],
  });
  ...
  if (!messageContent || typeof messageContent !== "string") {
    throw new HttpError(500, "Réponse IA vide");
  }
...
const getImageDataUrl = (imageResponse: any): string | null => {
  const directDataUrl = imageResponse?.choices?.[0]?.message?.images?.[0]?.image_url?.url;
  if (typeof directDataUrl === "string" && directDataUrl.startsWith("data:image")) {
...
export const injectInlineIllustrations = (
  markdown: string,
  illustrations: InlineIllustration[],
): string => {
